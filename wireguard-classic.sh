...
# ╠══════════════════════════════════════════════════════════════════════════╣
# ║                                                                          ║
# ║   Исправления v0.5 (относительно v0.4):                                  ║
# ║                                                                          ║
# ║   [БАГ 1] CLIENT_EXISTS / IPV4_EXISTS / IPV6_EXISTS: синтакс-ошибка      ║
# ║     grep -c при set -o pipefail может вернуть строку вида "0\n0"         ║
# ║     или строку с пробелами, что ломает арифметику [[ X -ne 0 ]].         ║
# ║     Исправлено: убираем пробелы/переводы строк, проверяем regex          ║
# ║     ^[0-9]+$, дефолт 0 при любом нечисловом значении.                    ║
# ║                                                                          ║
# ║   [БАГ 2] installQuestions: шаги [2/7] и [3/7] не ждали ввода            ║
# ║     until-цикл с предзаполненным валидным значением завершался           ║
# ║     немедленно без read, пользователь не мог изменить параметр.          ║
# ║     Переписаны на while true + read (всегда ждут ввода).                 ║
# ║                                                                          ║
# ║   [БАГ 3] DNS при установке: нельзя было ввести основной DNS             ║
# ║     Шаг [7/7] предлагал только резервный DNS; основной заранее           ║
# ║     устанавливался в "1.1.1.1" без диалога. Оба DNS теперь               ║
# ║     запрашиваются с показом дефолта и валидацией формата.                ║
# ║                                                                          ║
# ║   [УЛУЧШение] installQuestions: 7 шагов → 8, порт перенесён на [5/8]     ║
# ║     Порт выделен в отдельный видимый шаг; подсказки переформатированы    ║
# ║     в стиле «Параметр [дефолт] :» для единообразия.                      ║
# ║                                                                          ║
# ║   [УЛУЧШение] newClient: показ предлагаемого IP перед вводом             ║
# ║     Пользователь видит «Предлагаемый адрес: 10.66.66.2» и может          ║
# ║     принять его нажатием Enter или ввести другой.                        ║
# ║                                                                          ║
# ║   Запускать только с правами root:  bash wireguard-0_5.sh                ║
# ╚══════════════════════════════════════════════════════════════════════════╝
...
showClientQR() {
	...
	if command -v qrencode &>/dev/null; then
		...
		if qrencode -t png -l L -o "${PNG}" < "${FILE}" 2>/dev/null; then
			chmod 600 "${PNG}"
